/*
 * TicketMasterAPI class
 * will help us send HTTP request to TicketMaster API and get response, add some constants
 * */
package external;
...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "iE4iRTz5hGwDVfKts6cRGu30kME0evig";
	...
	private String getAddress(JSONObject event) throws JSONException {
		if (!event.isNull(EMBEDDED)) {
			...
			if (!embedded.isNull(VENUES)) {
				//api doc indicate that venues is an array
				...
				for (int i = 0; i < venues.length(); ++i) {
					...
					//in venues array, when we find one address, we return
					if (!addr.equals("")) {
						return addr;
					}
				}
	...
	private String getImageUrl(JSONObject event) throws JSONException {
		if (!event.isNull(IMAGES)) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				if (!image.isNull(URL_STR)) { //when we find one, we return
	...
	// queryAPI is used as a print function to show JSON array returned from TicketMaster.
	private void queryAPI(double lat, double lon) {
		List<Item> itemList = search(lat, lon, null);
		try {
			for (Item item : itemList) {
				//we did not implement print item, so we covert it to JSONObject
